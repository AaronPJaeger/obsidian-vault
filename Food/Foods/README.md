...
**Storage path:** `Food/Foods`  
**Daily logs:** `Food/Daily`

## How it works
1. Food files in this folder (`Food/Foods`) have frontmatter (`calories`, `protein`, `fat`, `carbs`, `serving_size`).
2. Daily log: put a `macros` code block in a note under `Food/Daily` (date id `YYYY-MM-DD`) and add foods/meals by name.
...
Macros plugin Settings → **Storage folder** = `Food/Foods`.
